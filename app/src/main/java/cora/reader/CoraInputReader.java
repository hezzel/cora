...
public class CoraInputReader {
   ...
   */
  private Term makeTerm(ParserTerm pt, Type expectedType, boolean typeShouldBeDerivable) {
    switch (pt) {
      ...
      case StringVal(Token t, String txt):
        ...
        try { ret = TheoryFactory.createEscapedStringValue(txt); }
          ...
          ret = TheoryFactory.createValue(txt);
        }
  ...
  private Term makeCalculationSymbol(Token token, String name, Type expected) {
    ...
    if (ret == null) { // this shouldn't happen: it's been created by the CoraParser
      throw new UnexpectedPatternError("CoraInputReader", "makeCalculationSymbol",
  ...
  private Term makeAbstraction(Token token, String varname, Type vartype, ParserTerm arg,
    ...
    if (_symbols.lookupFunctionSymbol(varname) != null) {
      ...
      storeError("Ambiguous binder: this name has already been declared as a free variable.",
        token);
    }
  ...
  static Term readTerm(String str, boolean constrained, SymbolData data, Type expected) {
    ...
    if (!pt.hasErrors()) {
      ...
      ret = reader.makeTerm(pt, null, true);
    }
    if (collector.queryErrorCount() > 0) {
      throw new ParseError(collector.queryCollectedMessages());
    }
    return ret;
  }
  ...
  public static Term readTerm(String str, TRS trs) {
    return readTerm(str, trs.isConstrained(), new SymbolData(trs), null);
  }
}
