...
\newcommand{\Reps}{\mathcal{R}\!\mathit{ep}}
...
We denote $\Terms(\Sigma)$ for the set of all true terms $s$, modulo $=_\alpha$.
In practice, we will reason with true terms rather than equivalence classes, but
always consider equality modulo $=_\alpha$.

The set of applicative terms is denoted $\ATerms(\Sigma)$.  Note that $=_\alpha$
is the identity on applicative terms, and that every applicative term is a true
term, so $\ATerms(\Sigma) \subseteq \Terms(\Sigma)$.

The set of first-order terms is denoted $\FOTerms(\Sigma)$.  Since every
first-order term is also an applicative pattern, $\FOTerms(\Sigma) \subseteq
\ATerms(\Sigma) \subseteq \Terms(\Sigma)$.

The set of theory terms is exactly $\Terms(\thSigma)$, as theory terms are
...
The set $\ATerms(\Sigma)$ is the smallest set such that:
\begin{itemize}
\item $\F \cup \Vfree \subseteq \ATerms(\Sigma)$;
\item if $s_1,\dots,s_n \in \ATerms(\Sigma)$ with $n \geq 2$ then
  $\tuple{s_1}{s_n} \in \ATerms(\Sigma)$;
\item if $s,t \in \ATerms(\Sigma)$ and $s :: \atype \arrtype \btype$ and $t ::
  \atype$ then $s \cdot t \in \ATerms(\Sigma)$.
\end{itemize}
\end{lemma}
...
\section{rules and rewriting}\label{sec:trs}

A rule $\rho$ is a pair $\ell \arrz r$ of two closed terms with the same type,
such that $\FMV(r) = \FMV(\ell)$, and $\ell$ is not a meta-variable application
$\meta{\Avar}{\ell_1,\dots,\ell_k}$ with $k \geq 0$ (so also not a variable).

For terms $s,t$, position $p \in \Positions(s,\mathsf{all})$ and rule $\rho =
\ell \arrz r$, we say that \emph{$s$ reduces to $t$ by rule $\rho$ at position
$p$} if there exists a substitution $\gamma$ such that $s|_p = \ell\gamma$ and
$t = s[r\gamma]_p$.
For $\Rules$ a set of rules, we say that \emph{$s$ reduces to $t$ by $\Rules$},
denoted $s \arr{\Rules} t$, if there exist a position $p \in \Positions(s,
\mathsf{partial})$ and rule $\rho \in \Rules$ such that $s$ reduces to $t$ by
rule $\rho$ at position $p$.

Note that this explicitly includes applications of rules at the head of a
subterm.

A rule $\ell \arrz r$ is:
\begin{itemize}
...
\begin{definition}
A \emph{generalised term rewriting system (TRS)} is an abstract rewriting
system of the form $(T_\F,\arr{\Rules})$, where $\F$ is an alphabet and $T_\F
\subseteq \Terms(\F,\V)$ is a set of true terms over $\F$, which is closed
under the reduction relation $\arr{\Rules}$.
\end{definition}

We consider the sets $\Sorts$, $\V$ and $\M$ implicit.  The use of a subset
$T_\F \subseteq \Terms(\F,\V)$ allows us to for instance consider first-order
(many-sorted) TRSs (where $T_\F = \FOTerms(\F,\V)$ and all rules are
first-order), applicative TRSs (where $T_\F = \ATerms(\F,\V)$ and all rules
are applicative), or TRSs where tuples and product types are not allowed in
term formation.
...
\begin{itemize}
\item Rules are represented by \texttt{charlie.trs.Rule} (although this class
  also represents \emph{constrained rules} -- see Section~\ref{sec:theories}).
  They can be queried for properties like left-linearity or being a pattern
  rule.
...
\item Both rules and generalised TRSs can be constructed using
  ...
  TRSs like many-sorted or pattern TRSs.  More about this in
  Section~\ref{sec:trskinds}.
...
\item More typically, a TRS is created with specific restrictions on term
  ...
  which properties the rules and rule schemes in a TRS satisfy.  More about this
  will also be explained in Section~\ref{sec:trskinds}.
\end{itemize}

==================

\section{Generalised TRSs with Theories}\label{sec:theories}

We fix a set $\thSorts \subseteq \Sorts$ of \emph{theory sorts}, and define a
\emph{theory type} as any type which only uses theory sorts; that is, a type
generated by the grammar:
\[
\thTypes ::= \thSorts \mid \thTypes \arrtype \thTypes \mid
  \product{\thTypes}{\thTypes}
\]
We additionally assume given, for every theory sort $\asort$, a set
$\I_\asort$.  For example, we may have $\thSorts = \{\tint,\bool\}$ and let
$\I_\tint = \mathbb{Z}$ and $\I_\bool = \{\top,\bot\}$.
We use this to generate an asosciated set for \emph{all} theory types:
\begin{itemize}
...
Next, we fix a set $\thF \subseteq \F$ of function symbols $(\afun :: \atype)$
where $\atype$ is a theory type, and a function $\J$ that maps each
$(\afun :: \atype) \in \thF$ to an element of $\I_\atype$.
We require that for every sort $\asort$, $\thF \supseteq \Val_\asort$, where
$\Val_\asort$ is a set of constants $\aconstructor :: \asort$ such that
$\J$ defines a bijection from $\Val_\asort$ and $\I_\asort$.  We call the
elements of $\Val := \bigcup_{\asort \in \thSorts} \Val_\asort$ \emph{values}.
All other elements of $\thF$ are called \emph{calculation symbols}.

A \emph{theory term} is one that uses only symbols from $\thF$, and
(meta-)variables of a theory type.  That is, if we
let $\thM := \{ (\Avar :: (\atype,k)) \mid (\Avar :: (\atype,k)) \in \M \wedge
\atype \in \thTypes \}$,
$\thVfree := \{ (\avar :: \atype) \mid (\avar :: \atype) \in \Vfree \wedge
\atype \in \thTypes \}$,
$\thVbound := \{ (\avar :: \atype) \mid (\avar :: \atype) \in \Vbound \wedge
\atype \in \thTypes \}$,
and $\thV := \thVfree \cup \thVbound$,
then a theory term is an element of $\MTerms(\thF,\thM,\thV)$.
Clearly, a theory term is still a term.

==================

Every closed theory term of type $\atype$ necessarily corresponds to an element
of $\I_\atype$, defined as follows:
\begin{itemize}
\item $\interpret{\afun} = \J_\afun$
\end{itemize}

\section{Different kinds of generalised TRSs}\label{sec:trskinds}
